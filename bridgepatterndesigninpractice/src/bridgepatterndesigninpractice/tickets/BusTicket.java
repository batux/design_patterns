...
public class BusTicket extends Ticket {
	...
	public BusTicket(String code, String number, Money baseFare,
			boolean domestic) {
	...
	public Money calculateTotalFare() {
		...
		if(!this.isDomestic()) {
			this.getBaseFare().setValue((this.getBaseFare().getValue() + (this.getBaseFare().getValue() * 0.11)));
		}
	...
	protected Money calculateTax() {
		...
		if(!this.isDomestic()) {
			totalTaxAmount.setValue((totalTaxAmount.getValue() * 0.018));
		}
